...
from mongoengine import Document as BaseDocument, connect
from fields import StringField, DecimalField
import uvicorn
import hu
import wtforms
...
class MyDocument(BaseDocument):  # Note: this should be different from subclassing
    ...
    def model(self):
        ...
        for name, field in self._fields.items():
            ...
            annos[name] = anno
        result = ModelMetaclass("ItemPlus", (BaseModel, ), dict(__annotations__=annos, **defaults))
        return result
...
def create_item(item: ItemPlus):
    """
    Sample POST method adds input to MongoDB after conversion to a suitable Document subclass.

    uvicorn POST methods take a dictionary (constructed from input of type 'application/json')
    and convert it into the type annotated in the function signature. This ensures that
    only valid objects can be created. Note that uvicorn is happy to accept a dictionary
...
#
# Does not work: can't figure out how to handle structures in forms.
# Answer may lie in subclassing or perhaps composing.
#
#def process_form(item: Item=apiForm(...)):
    #print(form)
    #return {}

def process_form(name: str=apiForm(...), description: str=apiForm(...), price: Decimal=apiForm(...)):
    print(name, description, price)
    item = Item(name=name, description=description, price=price)
    form = MongoItem.form()(MultiDict({'name': name, 'description': description, 'price': price}))
    print(form.validate())
    return item
...
def item_for_display(item_id: str) -> HTMLResponse:
    ...
    """
    result = MongoItem.objects(name=item_id).get()
    return HTMLResponse(f"<html><body><h1>{result.description}</h1><p>{result.name}: {result.price}</p></body></html>")
